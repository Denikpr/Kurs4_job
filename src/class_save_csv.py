...
class Saver:
    ...
    def save_vacancies_csv(self, vacancies: list):
        ...
        with open(path, 'w', newline='', encoding='UTF-8') as file:
            ...
            writer.writerow(
                ['id', 'name', 'data_published', 'salary_from', 'salary_to', 'currency', 'area', 'url', 'employer',
                 'employer_url', 'requirement', 'experience', 'employment'])
            for vacancy in vacancies:
                writer.writerow([vacancy.id, vacancy.name, datetime.datetime.fromtimestamp(vacancy.data_published).
                                strftime('%Y-%m-%d %H:%M:%S'), vacancy.salary_from, vacancy.salary_to, vacancy.currency,
                                 vacancy.area, vacancy.url, vacancy.employer, vacancy.employer_url, vacancy.requirement,
